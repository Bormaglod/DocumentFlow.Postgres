...
CREATE OR REPLACE FUNCTION public.goods_balance_receipt(document_id uuid, doc_kind uuid, doc_number character varying, ref_id uuid, amount numeric, cost numeric, receipt_date timestamp with time zone) RETURNS void
    LANGUAGE plpgsql
    AS $$
declare
	kind_name varchar;
...
ALTER FUNCTION public.goods_balance_receipt(document_id uuid, doc_kind uuid, doc_number character varying, ref_id uuid, amount numeric, cost numeric, receipt_date timestamp with time zone) OWNER TO postgres;

COMMENT ON FUNCTION public.goods_balance_receipt(document_id uuid, doc_kind uuid, doc_number character varying, ref_id uuid, amount numeric, cost numeric, receipt_date timestamp with time zone) IS 'Приход материала
