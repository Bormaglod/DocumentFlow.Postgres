...
begin
	for var_pp in
		select 
			pp.*,
			pp.tableoid::regclass::varchar as table_name
	...
	loop 
		...
		if (var_pp.table_name = 'posting_payments_purchase') then
		...
		elsif (var_pp.table_name = 'posting_payments_receipt') then
			table_name = 'receipt';
